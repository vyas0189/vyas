...
test.describe('Homepage', () => {
	...
	test('should not have console errors', async ({ page }) => {
		...
		page.on('console', (msg) => {
			...
			const text = msg.text();
			// Ignore Vite dev-server artifacts that don't exist in production.
	...
	test('should load without network errors', async ({ page }) => {
		...
		page.on('requestfailed', (request) => {
			...
			// Ignore Astro/Vite dev-only URLs that never ship to production.
			if (url.includes('/_image')) return;
			if (url.includes('/@id/')) return;
			if (url.includes('/@vite/')) return;
			if (url.includes('/__vite_ping')) return;
